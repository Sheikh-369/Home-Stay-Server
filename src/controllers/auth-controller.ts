...
class AuthController {
  ...
  static async userRegister(req: Request, res: Response) {
    //validation
    const{
      userName,userEmail,phoneNumber,userPassword,confirmPassword,city,district,country} = req.body;
    if (!userName || !userEmail || !userPassword || !confirmPassword || !phoneNumber || !city || !district || !country) {
      res.status(400).json({
  ...
  static async forgotPassword(req: Request, res: Response) {
    ...
    sendEmail({
      ...
      html: `<div style="font-family: Arial, sans-serif; max-width: 400px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; background: #ffffff;">
            ...
            <p style="font-size: 12px; color: #777;">Thanks,<br>PGStay<br>Chabehil, Kathmandu</p>
        </div>`,
    });
...
//   //update profile
  static async updateProfile(req: Request, res: Response) {
    const { id } = req.params;
    const { 
      userName, city, country,phoneNumber, 
